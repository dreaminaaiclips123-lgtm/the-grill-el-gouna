...
- **Signature interaction, v3 (rejected):** `ScrollIgnite`, a pinned scroll-scrubbed version of the ignition video (scrolling drove `video.currentTime` directly). Client found the scroll-scrub itself bad and it didn't reliably play through to the end; removed. Do not reintroduce scroll-scrubbing here.
- **Signature interaction, v4 (current, opening moment):** `IntroVideo` is a full-screen, unskippable, autoplaying overlay shown on every fresh load of the homepage, above `Nav`. It plays `public/videos/ignite.mp4` (Higgsfield `seedance_2_5`, 9:16, ~8s, charcoal igniting into full flame) start to finish with no controls and no skip affordance; the "The Grill" wordmark fades in and back out over the first ~2.4s so it doesn't compete with the rest of the clip. On the video's native `ended` event (with a 12s safety timeout as a fallback if playback stalls) the whole overlay cross-fades into the homepage beneath it, which is already mounted and rendering underneath, not delayed. Body scroll is locked while it plays. `prefers-reduced-motion` skips the video entirely and shows a ~0.9s static poster instead, never a forced 8s clip.
