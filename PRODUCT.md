...
- **Mobile glitch, round 3 (client: "its fucked even more"):** the round-2 fix made things worse, for a concrete, findable reason: gating the whole overlay behind a client-only `ready` flag meant the real homepage was now guaranteed to flash on screen for one frame *before* the intro even appeared, on every load, every device — worse than the original bug, not a fix for it. Rewrote again to: (a) render the overlay unconditionally from the very first paint on both server and client (same initial `useState` values either side, so still zero hydration-mismatch risk, without the flash-of-real-content regression), and (b) gate the hold/fade countdown on the badge image's actual `onLoad` event (with a 2.5s safety-timeout fallback) instead of starting blindly on mount, so a slow mobile connection can't cause the image to pop in mid-transition. **Tooling limitation discovered while debugging this:** the browser-preview tool used to test throttles `requestAnimationFrame`/`setTimeout` heavily on background (unfocused) tabs — one frame-by-frame capture attempt got exactly one animation frame across 48 real seconds while the tab was unfocused. This invalidated most of the timing-based verification attempted during rounds 2 and 3; a fronted/focused tab is required for any timing-sensitive check here, and even then, sub-second visual glitches on a real device (especially iOS Safari specifically) cannot be fully verified through this tool. Trust careful code review over automated timing capture for this class of bug.
